...
export class MenuAPI {
   ...
   * 获取所有菜单项
   */
  static async getMenuItems(page: number = 1, limit: number = 20, category?: string): Promise<PaginatedResponse<MenuItem>> {
    ...
    try {
      const response = await httpClient.get<PaginatedResponse<MenuItem>>(`/menu/items?${params}`)
      return response
    } catch (error) {
